...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
	#warning "should this be codecoin/blu/etc"
        if (boost::algorithm::istarts_with(argv[i], "bitcoin:"))
        {
            const char *strURI = argv[i];
            try {
                boost::interprocess::message_queue mq(boost::interprocess::open_only, CODECOINURI_QUEUE_NAME);
...
static void ipcThread2(void* pArg)
{
    ...
    message_queue::remove(CODECOINURI_QUEUE_NAME);
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        mq = new message_queue(open_or_create, CODECOINURI_QUEUE_NAME, 2, MAX_URI_LENGTH);
