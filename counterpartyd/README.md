# Changelog
* v0.1.0.1-import into catcoin catbox
...
payments. It uses catcoin as a transport layer. The contents of this
...
running instance of catcoind or catcoin-Qt, which handles many catcoin‐specific
matters on its behalf, including all wallet and private key management. For
such interoperability, catcoind must be run with the following options:
`-txindex=1` `-server=1` and, as desired, `-testnet=1`. This may require
the setting of a JSON‐RPC password, which may be saved in catcoind’s
configuration file.

counterpartyd needs to know at least the JSON‐RPC password of the catcoind with
...
Note that the syntaxes for the countpartyd and the catcoind configuraion
files are not the same. A catcoind configuration file looks like this:
...
If and only if counterpartyd is to be run on the catcoin testnet, with the
`--testnet` CLI option, catcoind must be set to do the same (`-testnet=1`).
...
* Server
	...
	The `server` command should always be running in the background. All other commands will fail if the index of the last block in the database is less than that of the last block seen by catcoind.
...
* Market
	...
	The `market` action prints out tables of open orders, open bets, feeds, and order matches currently awaiting catcoin payments from one of your addresses. It is capable of filtering orders by assets to be bought and sold.
