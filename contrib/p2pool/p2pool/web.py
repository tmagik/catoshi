...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event()):
    ...
    def get_patron_sendmany(total=None, trunc='0.01'):
        ...
        total = int(float(total)*node.net.PARENT.MULTIPLIER)
        trunc = int(float(trunc)*node.net.PARENT.MULTIPLIER)
        return json.dumps(dict(
            (bitcoin_data.script2_to_address(script, node.net.PARENT), value/node.net.PARENT.MULTIPLIER)
    ...
    def get_local_stats():
        ...
        return dict(
            ...
            block_value=node.bitcoind_work.value['subsidy']*node.net.PARENT.MULTIPLIER,
    ...
    web_root.putChild('current_payouts', WebInterface(lambda: dict((bitcoin_data.script2_to_address(script, node.net.PARENT), value/node.net.PARENT.MULTIPLIER) for script, value in node.get_current_txouts().iteritems())))
    ...
    def update_stat_log():
        ...
        stat_log.append(dict(
            ...
            current_payout=node.get_current_txouts().get(bitcoin_data.pubkey_hash_to_script2(wb.my_pubkey_hash), 0)*node.net.PARENT.MULTIPLIER,
            peers=dict(
                incoming=sum(1 for peer in node.p2p_node.peers.itervalues() if peer.incoming),
                outgoing=sum(1 for peer in node.p2p_node.peers.itervalues() if not peer.incoming),
            ),
        ...
        return dict(
            ...
            block=dict(
                ...
                gentx=dict(
                    ...
                    value=share.share_data['subsidy']*node.net.PARENT.MULTIPLIER,
    ...
    def add_point():
        ...
        hd.datastreams['current_payout'].add_datum(t, current_txouts.get(bitcoin_data.pubkey_hash_to_script2(wb.my_pubkey_hash), 0)*node.net.PARENT.MULTIPLIER)
        miner_hash_rates, miner_dead_hash_rates = wb.get_local_rates()
        ...
        hd.datastreams['current_payouts'].add_datum(t, dict((user, current_txouts_by_address[user]*node.net.PARENT.MULTIPLIER) for user in miner_hash_rates if user in current_txouts_by_address))
